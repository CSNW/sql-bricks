(function() {
  ...
  [Select, Insert, Update, Delete].forEach(function(stmt) {
    stmt.defineClause = function(clause_id, template, opts) {
      ...
      if ( typeof templ_fn != 'function' )
        templ_fn = function(opts) { return sql.templ(template, this, opts); };
  ...
  // SELECT statement
  sql.select = sql.inherits(Select, Statement);
  ...
  Select.prototype._aliasToString = function(opts) {
    ...
    return ' ' + sql._autoQuote(this._alias);
  };
  ...
  Select.defineClause('from', function(opts) {
    if ( !this._from )
      return;
    var result = 'FROM ' + sql._handleTables(this._from, opts);
    if ( this.joins )
  ...
  // INSERT statement
  sql.insert = sql.insertInto = sql.inherits(Insert, Statement);
  ...
  Insert.defineClause('columns', function(opts) {
    ...
    return '(' + sql._handleColumns(_.keys(this._values[0]), opts) + ')';
  });
  Insert.defineClause('values', function(opts) {
    if ( this._select ) {
      ...
      return 'VALUES ' + _.map(this._values, function(values) {
        return '(' + sql._handleValues(_.values(_.pick(values, pickOrder)), opts).join(', ') + ')';
      }).join(', ');
    }
  });
  ...
  // UPDATE statement
  sql.update = sql.inherits(Update, Statement);
  ...
  Update.defineClause('set', function(opts) {
    return 'SET ' + _.map(this._values, function(value, key) {
      return sql._handleColumn(key, opts) + ' = ' + sql._handleValue(value, opts);
    }).join(', ');
  });
  ...
  // DELETE statement
  sql.delete = sql.deleteFrom = sql.inherits(Delete, Statement);
  ...
  Join.prototype.toString = function toString(opts) {
    var on = this.on, tbl = sql._handleTable(this.tbl, opts);
    ...
    if ( !on || _.isEmpty(on) ) {
      if ( sql._joinCriteria ) {
        var left_tbl = sql._handleTable(this.left_tbl, opts);
    ...
    if ( _.isArray(on) ) {
      on = _.map(on, function(column) {
        return sql._handleColumn(column);
      }).join(', ');
    ...
    if ( isExpr(on) ) {
      ...
      on = _.map(_.keys(on), function(key) {
        return sql._handleColumn(key, opts) + ' = ' + sql._handleColumn(on[key], opts);
      }).join(' AND ');
    }
  ...
  Binary.prototype.toString = function toString(opts) {
    var _sql = sql._handleColumn(this.col, opts);
    return _sql + ' ' + this.op + ' ' + this.quantifier + sql._handleValue(this.val, opts);
  };
  ...
  Like.prototype.toString = function toString(opts) {
    var _sql = sql._handleColumn(this.col, opts) + ' LIKE ' + sql._handleValue(this.val, opts);
    if ( this.escape_char )
      _sql += " ESCAPE '" + this.escape_char + "'";
    return _sql;
  };
  ...
  Between.prototype.toString = function(opts) {
    return sql._handleColumn(this.col, opts) + ' BETWEEN ' + sql._handleValue(this.val1, opts) + ' AND ' + sql._handleValue(this.val2, opts);
  };
  ...
  Unary.prototype.toString = function toString(opts) {
    return sql._handleColumn(this.col, opts) + ' ' + this.op;
  };
  ...
  In.prototype.toString = function toString(opts) {
    var col_sql = sql._handleColumn(this.col, opts);
    var _sql;
    if ( _.isArray(this.list) )
      _sql = sql._handleValues(this.list, opts).join(', ');
    else if ( this.list instanceof Statement )
      _sql = this.list._toString(opts);

    return col_sql + ' IN (' + _sql + ')';
  };
  ...
  sql._handleExpression = function(expr, opts) {
    expr.parens = false;
  ...
  sql._handleValues = function(vals, opts) {
    return vals.map(function(val) {
      return sql._handleValue(val, opts);
    });
  };

  sql._handleValue = function(val, opts) {
  ...
  sql._handleTables = function(tables, opts) {
    return tables.map(function(tbl) { return sql._handleTable(tbl, opts); }).join(', ');
  };

  sql._handleTable = function(table, opts) {
    return sql._handleColumn(expandAlias(table), opts);
  };

  sql._handleColumns = function(cols, opts) {
    return cols.map(function(col) { return sql._handleColumn(col, opts); }).join(', ');
  };
  ...
  sql._handleColumn = function(expr, opts) {
    if ( expr instanceof Statement )
      return expr._toNestedString(opts);

    if ( expr instanceof val )
      return sql._handleValue(expr.val, opts);

    if ( expr instanceof sql )
      return expr.toString(opts);

    if ( unquoted_regex.test(expr) )
      return sql._quoteColOrTbl(expr);
    else
      return expr;
  };

  sql._quoteColOrTbl = function(expr) {
    var prefix = '';
    ...
    return (prefix ? sql._autoQuote(prefix) + '.' : '') + sql._autoQuote(expr) + suffix;
  };

  // auto-quote tbl & col names if they have caps or are reserved words
  sql._autoQuoteChar = '"';

  sql._autoQuote = function(str) {
  ...
  sql._aliases = {};
  sql.aliasExpansions = function(aliases) {
  ...
  sql.templ = function(str, ctx, opts) {
    var result = '';
    var lastIndex = 0;
  ...
  sql.templ.helpers = {
    'if': function(val, opts, contents, ctx) { return val ? sql.templ(contents, ctx, opts) : ''; },
    'ifNotNull': function(val, opts, contents, ctx) { return val != null ? sql.templ(contents, ctx, opts) : ''; },
    'columns': sql._handleColumns,
